...
namespace MHZNet.Core.SeedData
{
    /// <summary>
    /// 
    /// </summary>
    public class SeedService
    {
            ...
            TenantOptions tenantOptions)
        {
            try
            {
                ConsoleHelper.WriteLine($"开始初始化数据....", ConsoleColor.Green);
                ConsoleHelper.WriteLine($"是否开发调试: {isQuickDebug}", ConsoleColor.Green);
                ...
                ConsoleHelper.WriteLine("开始创建数据库....", ConsoleColor.Green);
                ...
                {
                    //可能有问题，如有异常请注释掉
                    throw new Exception("不支持Oracle使用代码建库,请先建库注释该代码重新运行");
                }

                ConsoleHelper.WriteLine("开始创建数据库成功！", ConsoleColor.Green);
                ConsoleHelper.WriteLine("开始初始化数据表....");
                ...
                ConsoleHelper.WriteLine("开始初始化数据表成功！", ConsoleColor.Green);
                ConsoleHelper.WriteLine();
                ...
                if (isInitData)
                {
                    ConsoleHelper.WriteLine("开始初始化种子数据....");
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<User>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<User>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(User)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<Role>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<Role>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(Role)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<Menu>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<Menu>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(Menu)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<Department>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<Department>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(Department)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<Job>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<Job>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(Job)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<Setting>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<Setting>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(Setting)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<Dict>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<Dict>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(Dict)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<DictDetail>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<DictDetail>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(DictDetail)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<QuartzNet>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<QuartzNet>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(QuartzNet)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<EmailAccount>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<EmailAccount>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(EmailAccount)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<EmailMessageTemplate>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<EmailMessageTemplate>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(EmailMessageTemplate)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<UserRole>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<UserRole>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(UserRole)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<UserJob>()
                                .InsertRangeAsync(JsonConvert.DeserializeObject<List<UserJob>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(UserJob)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<RoleMenu>()
                                .InsertRangeAsync(JsonConvert.DeserializeObject<List<RoleMenu>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(RoleMenu)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<Apis>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<Apis>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(Apis)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<RoleApis>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<RoleApis>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(RoleApis)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                        ...
                        if (attr != null)
                        {
                            await dataContext.GetEntityDb<Tenant>().InsertRangeAsync(
                                JsonConvert.DeserializeObject<List<Tenant>>(
                                    ...
                                    setting));
                            ConsoleHelper.WriteLine(
                                $"Entity:{nameof(Tenant)}-->Table:{attr.TableName}-->Desc:{attr.TableDescription}-->初始化数据成功！",
                                ConsoleColor.Green);
                        }
                    }
                    ...
                    ConsoleHelper.WriteLine("初始化种子数据完成！", ConsoleColor.Green);
                    ConsoleHelper.WriteLine();
                }
        ...
        public static Task InitLogData(DataContext dataContext, string logDataBase)
        {
            ...
            {
                throw new ApplicationException("未配置日志数据库，请检查appsettings.json中DataConnection节点配置");
            }
            ...
            {
                //���п������� �����쳣����ע�͵�
                throw new Exception("sqlSugar不支持Oracle使用代码建库,请先建库注释该代码重新运行");
            }

            ConsoleHelper.WriteLine("初始化日志数据库成功！", ConsoleColor.Green);
            ConsoleHelper.WriteLine("初始化日志数据表....");
            ...
            ConsoleHelper.WriteLine("初始化日志数据表成功！", ConsoleColor.Green);
            ConsoleHelper.WriteLine();
        ...
        public static async Task InitTenantDataAsync(DataContext dataContext)
        {
            ...
            foreach (var tenant in tenants)
            {
                ...
                ConsoleHelper.WriteLine($"初始化租户{tenant.Name}成功！", ConsoleColor.Green);
            ...
            ConsoleHelper.WriteLine("初始化租户数据完成！", ConsoleColor.Green);
        }
    }
}
