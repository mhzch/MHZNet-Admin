...
namespace MHZNet.Common.Helper;
...
public class CmdHelper
{
    ...
    public static string Run(string cmdText, string cmdPath = "cmd.exe")
    {
        ...
        //说明：不管命令是否成功均执行exit命令，否则当调用ReadToEnd()方法时，会处于假死状�? 
        var cmd = cmdText + " &exit";
        using (var p = new Process())
        {
            p.StartInfo.FileName = cmdPath;
            p.StartInfo.UseShellExecute = false; //是否使用操作系统shell启动  
            p.StartInfo.RedirectStandardInput = true; //接受来自调用程序的输入信�? 
            p.StartInfo.RedirectStandardOutput = true; //由调用程序获取输出信�? 
            p.StartInfo.RedirectStandardError = true; //重定向标准错误输�? 
            p.StartInfo.CreateNoWindow = true; //不显示程序窗�? 
            p.Start(); //启动程序  
    ...
    /// <summary>
    /// Linux操作系统，执行Shell
    /// �?using https://github.com/phil-harmoniq/Shell.NET �?    /// </summary>
